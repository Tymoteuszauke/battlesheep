...
(cd ../client/target/sonar ; xdg-open report-task.txt)
(cd ../common/target/sonar ; xdg-open report-task.txt)
(cd ../server/target/sonar ; xdg-open report-task.txt)
...
echo ' '
echo 'RESULTS FROM PLUGIN ARE IN [client|server]/target/site/'
echo ' '